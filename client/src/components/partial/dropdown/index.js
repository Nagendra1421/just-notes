...
export default class Drop extends PureComponent {
  ...
  render() {
    const { props: { triger, icon, right, children } } = this;
    return (
      <div className="drop">
        <button type="button" onClick={this.toggle} className={`drop-triger${triger ? ` ${triger}` : ''}`}>
        {icon ? <i className={icon} /> : null}
        </button>
        <div className={`drop-menu${right ? ' menu-right' : ''}${this.state.show ? ' show': ''}`}>
          {children}
        </div>
      </div>
    )
